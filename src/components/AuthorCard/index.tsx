...
const AuthorCard = ({ author }: { author: Author }) => {
  return (
    <Link
      ...
      className="card w-[140px] sm:w-[197px] h-[176px] sm:h-[247px] bg-white shadow-sm group border border-black06 hover:shadow-lg"
    >
...
const Frame = ({ name, src }: { name: string; src: string | null }) => {
  ...
  return (
    <div className="bg-black05 mask mask-squircle p-[4px] w-24 sm:w-32 h-24 sm:h-32 block mt-[30px] sm:mt-[42px] mx-auto">
      <figure className="relative mask mask-squircle w-full h-full">
        <Image
          src={src ?? defaultSrc}
          alt={name[0]}
          fill
